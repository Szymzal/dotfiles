{
  ...
  lib.myLib = rec {
    ...
    mkThrowNull = item: message: (mkThrow (item != null) message);
    attrToList = str: lib.strings.splitString "." str;
    getOptionsFromHomeConfigs = configPath: lib.attrsets.mapAttrsToList (_: value: (lib.attrsets.getAttrFromPath (attrToList configPath) value)) config.home-manager.users;
    isEnabledOptionOnHomeConfig = option: (lib.lists.findSingle (value: value) false true (getOptionsFromHomeConfigs option));
    hyprlandMonitorsConfig = lib.forEach config.mypackages.monitors.config (
      value:
        if value.enable
